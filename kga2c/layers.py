...
class PackedEncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        lengths = torch.tensor(
            [torch.nonzero(n)[-1] + 1 for n in input], dtype=torch.long
        ).to(
            device
        )  # , device=device)
        ...
        idx = (
            (lengths - 1).view(-1, 1).expand(len(lengths), output.size(2)).unsqueeze(0)
        )
...
class DecoderRNN2(nn.Module):
    def __init__(self, hidden_size, output_size, embeddings, graph_dropout):
        ...
        self.gru = nn.GRU(hidden_size, hidden_size)
        self.out = nn.Linear(hidden_size, output_size)
        self.graph_dropout = UnregDropout(
            p=graph_dropout
...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=False):
        ...
        self.a = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.Tensor(2 * out_features, 1).type(
                    torch.cuda.FloatTensor
                    if torch.cuda.is_available()
                    else torch.FloatTensor
                ),
...
class EncoderLSTM(nn.Module):
    def __init__(
        ...
        self.embedding = (
            embeddings  # nn.Embedding(vocab_size, embedding_size, padding_idx)
        )
        self.gru = nn.LSTM(
            embedding_size,
            hidden_size,
