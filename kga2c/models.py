...
class ObjectDecoder(nn.Module):
    def __init__(
        self, input_size, hidden_size, output_size, embeddings, graph_dropout, k
    ):
    ...
    def forward(self, input, input_hidden, vocab, vocab_rev, decode_steps_t, graphs):
        ...
        for di in range(self.max_decode_steps):
            ret_decoder_output, decoder_output, decoder_hidden = self.decoder(
                decoder_input, decoder_hidden, input, graphs
            )
            ...
            if self.k == 1:
                ...
                for i in range(graphs.size(0)):
                    cur_obj = np.random.choice(
                        topi[i].reshape(-1), p=topv[i].reshape(-1)
                    )
...
class KGA2C(nn.Module):
    def __init__(
        ...
        self.decoder_object = ObjectDecoder(
            50,
            100,
            len(self.vocab.keys()),
            self.action_emb,
    ...
    def get_action_rep(self, action):
        ...
        for i, token in enumerate(action.split()[:20]):
            ...
            action_desc_num[i] = (
                self.vocab_rev[short_tok] if short_tok in self.vocab_rev else 0
            )
    ...
    def forward(self, obs, scores, graph_rep, graphs):
        ...
        decoder_t_output, decoder_t_hidden = self.decoder_template(
            state_emb, h_t
        ...
        for i in range(batch):
            ...
            templ, decode_step = self.get_action_rep(
                self.templates[topi[i].squeeze().detach().item()]
            )
        ...
        return (
            ...
            value,
            decode_steps,
        )  # decoder_t_output#template_mask
...
class StateNetwork(nn.Module):
    def __init__(
        ...
        self.state_ent_emb = nn.Embedding.from_pretrained(
            torch.zeros((len(self.vocab_kge), self.embedding_size)), freeze=False
        )
