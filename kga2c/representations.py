...
from kga2c.corenlp import corenlp_annotate
...
class StateAction(object):
    ...
    def visualize(self):
        ...
        nx.draw(
            self.graph_state, pos=pos, with_labels=True, node_size=200, font_size=10
        )
        # plt.show()
    ...
    def update_state(
        ...
        con_cs = [
            graph_copy.subgraph(c) for c in nx.weakly_connected_components(graph_copy)
        ]
        ...
        sents = corenlp_annotate(self.visible_state)
        ...
        for rule in add_rules:
            ...
            if (
                u in self.vocab_kge["entity"].keys()
    ...
    def get_state_rep_kge(self):
        ...
        for u, v in self.graph_state.edges:
            ...
            if (
                u not in self.vocab_kge["entity"].keys()
                or v not in self.vocab_kge["entity"].keys()
            ):
                break
    ...
    def step(
        ...
        ret, ret_cache = self.update_state(
            visible_state, inventory_state, objs, prev_action
        )
